...
Think through your response step by step and provide detailed explanations."""
        ...
        logger.debug(f"Generated free-form text (first {200} chars): {free_form_text[:200]}...")
    ...
    def _convert_to_json(
        ...
        example_instance = target_model_class.example() if hasattr(target_model_class, 'example') else {}
...
JSON OUTPUT:"""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.debug(f"Successfully converted to structured JSON on attempt {attempt + 1}")
                return validated_model
                
            except Exception as e:
                logger.warning(f"JSON conversion attempt {attempt + 1} failed: {e}")
                if attempt == max_retries:
                    logger.error(f"Failed to convert to JSON after {max_retries + 1} attempts")
                    raise ValueError(f"Failed to convert free-form text to structured JSON: {e}")
                else:
...
def create_two_step_system_prompt(original_system_prompt: str) -> str:
    ...
    json_keywords = [
        "json", "JSON", "structured format", "format your response",
        "```json", "output format", "structure your output"
    ]
