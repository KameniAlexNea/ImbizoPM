...
def refine_project_idea(project_idea, model_name, api_key=None):
    ...
    try:
        ...
        if "API key" in error_msg.lower():
            ...
            return None, f"Connection error: Could not connect to the model service. Is Ollama running? Error: {error_msg}"
        else:
    ...
    with gr.Blocks() as iface:
        ...
        def update_model_visibility(model_name):
            ...
            return {
                ...
                model_info: gr.update(value=f"**Model Info**: {model_desc}" if not is_custom else "")
            }
        ...
        def process_refinement(
            ...
            model_name = custom_model if model_selection == "custom" else model_selection
            ...
            if not feedback or not refined_state:
                ...
                yield {
                    output_status: gr.update(value="### ✅ Initial Refinement Complete"),
            ...
            # Refinement with feedback
                ...
                if not refined_state:
                    yield {
                        output_status: gr.update(value="### ❌ Error: Missing previous refinement"),
                        submit_button: gr.update(interactive=True)
                    }
                    return project_state, refined_state, history_state
                ...
                yield {
                    output_status: gr.update(value="### ✅ Refinement Updated with Feedback"),
                    output_text: gr.update(value=result),
                    history_container: gr.update(value=history_html),
                    submit_button: gr.update(interactive=True)
                }
        ...
        def generate_history_html(history):
            ...
            for i, entry in enumerate(history):
                ...
                html += f"<h4>Refinement #{len(history) - i} - {entry['timestamp']}</h4>"
