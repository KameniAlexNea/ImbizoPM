...
class PlannerAgent(BaseAgent):
    ...
    def _prepare_input_logic(self, state: AgentState) -> str:
        ...
        if backward_route == AgentRoute.NegotiatorAgent:
            ...
            prompt_parts.append(f"# Risk Assessment Feedback (Dealbreakers):\n{dumps_to_yaml(dealbreakers, indent=4)}")
            feedback_added = True
        elif backward_route == AgentRoute.ValidatorAgent:
            validator_output = state.get(AgentRoute.ValidatorAgent)
            validation_issues = getattr(validator_output, 'completeness_assessment', "No details provided")  # Assuming this holds issues
            prompt_parts.append(f"# Validation Feedback:\n{dumps_to_yaml(validation_issues, indent=4)}")
            feedback_added = True
        ...
        if feedback_added:
            ...
            prompt_parts.append("Based on the clarified details, break the project into phases, epics, and strategies.")
    ...
    def _next_step_logic(self, state: AgentState, result: AgentDtypes.PlannerAgent) -> AgentRoute:
