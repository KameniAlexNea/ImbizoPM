...
class TestMultiProviderGenerator(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertIs(generator.llm, generator.llm_providers[0])  # Default master is first
        
        # Test with invalid master index
        with self.assertRaises(ValueError):
            MultiProviderProjectGenerator(
                providers=[self.provider1, self.provider2],
                master_provider_idx=5
            )
        ...
        with self.assertRaises(ValueError):
            MultiProviderProjectGenerator(
                ...
                provider_kwargs=[{"api_key": "key1"}]  # Only one config for three providers
            )
    ...
    def test_generate_project_description_with_single_provider(self):
        ...
        self.assertNotIn("_aggregate_descriptions", description)  # No aggregation with single provider
    ...
    def test_generate_project_description_with_multiple_providers(self):
        ...
        description = self.generator.generate_project_description("Create a task manager")
    ...
    def test_aggregate_descriptions(self):
        ...
        result = self.generator._aggregate_descriptions(["Single description"], "prompt")
    ...
    def test_aggregate_tasks(self):
        ...
        with self.assertRaises(ValueError):
            self.generator._aggregate_tasks([{"project_title": "Empty", "tasks": []}], "description")
