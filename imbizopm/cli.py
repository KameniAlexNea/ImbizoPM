...
def parse_args():
    ...
    ai_project_parser.add_argument("--repo", help="Repository name (if not provided, will be extracted from generated title)")
    ...
    ai_project_parser.add_argument("--model", help="Model name for the selected provider")
    ai_project_parser.add_argument(
        "--private", action="store_true", help="Make repository private"
    )
    ai_project_parser.add_argument(
    ...
    ai_project_parser.add_argument(
        "--dry-run", action="store_true", help="Generate tasks without creating GitHub project"
    )
        ...
        if args.command == "ai-project":
            ...
            try:
                ...
                if not repo_result["success"]:
                    print(f"Failed to create repository: {repo_result.get('error', 'Unknown error')}")
                    sys.exit(1)
